...
import { ArrowLeft, Activity, Heart, Droplets, TrendingUp, FileText, Upload, ShieldCheck, CheckCircle2, ShieldAlert } from 'lucide-react';
...
const PatientDetail = () => {
  ...
  // --- 🏆 PROFESSIONAL PDF (RESTORED TO FULL VERSION) ---
  ...
  const handleFileUpload = async (e) => {
    ...
    setUploading(true);
    try {
      await api.post(`/patients/${id}/upload`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      fetchRecords();
      alert("Sync Success!");
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20 p-6">
      ...
      <div className="flex justify-between items-center">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-400 hover:text-slate-900 font-bold uppercase text-[10px] tracking-widest"><ArrowLeft size={16} /> Dashboard</button>
        <button onClick={downloadProfessionalReport} className="flex items-center gap-2 px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-emerald-700 transition-all shadow-emerald-200"><FileText size={18} /> Export Clinical PDF</button>
      </div>
      ...
      <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
        <div className="flex items-center gap-8 z-10">
          <div className={`p-8 rounded-[2.5rem] shadow-2xl ${patient.lastHeartRate > 100 ? 'bg-red-600' : 'bg-blue-600'} text-white shadow-blue-200`}><Activity size={48} /></div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-50">
            <div className="flex items-center justify-between mb-4"><Heart className="text-red-500" size={24} fill="currentColor"/><span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Pulse Rate</span></div>
            <div className={`text-6xl font-black italic ${patient.lastHeartRate > 100 ? 'text-red-600' : 'text-slate-800'}`}>{patient.lastHeartRate.toFixed(1)} <span className="text-xl italic font-bold text-slate-300">BPM</span></div>
          </div>
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-50">
            <div className="flex items-center justify-between mb-4"><Droplets className="text-blue-500" size={24} fill="currentColor"/><span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">SpO2 Level</span></div>
            <div className={`text-6xl font-black italic ${patient.lastOxygenLevel < 95 ? 'text-red-600' : 'text-slate-800'}`}>{patient.lastOxygenLevel.toFixed(1)}<span className="text-xl italic font-bold text-slate-300">%</span></div>
          </div>
        </div>

        {/* 📉 MULTI-LINE SMOOTH CHART (FIXED) */}
      ...
      <div className="bg-slate-900 p-12 rounded-[4rem] shadow-2xl text-white">
        <div className="flex justify-between items-center mb-10 border-b border-slate-800 pb-8">
          <div><h2 className="text-4xl font-black uppercase tracking-tighter italic">Clinical Vault</h2><p className="text-slate-500 text-[10px] font-bold tracking-widest uppercase mt-1">Diagnostic Artifact Storage</p></div>
          <label className="cursor-pointer flex items-center gap-3 px-8 py-4 bg-blue-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20"><Upload size={18} /> {uploading ? 'Encrypting...' : 'Add Record'}<input type="file" className="hidden" onChange={handleFileUpload} /></label>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {records.map((rec) => (
            <div key={rec.id} onClick={() => window.open(`http://localhost:8080/api/patients/record/${rec.id}`, '_blank')} className="p-6 bg-slate-800 rounded-[2.5rem] border border-slate-700 flex items-center gap-4 group hover:border-blue-500 transition-all cursor-pointer">
              <div className="p-4 bg-slate-700 rounded-2xl text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-all"><FileText size={24} /></div>
              <div className="overflow-hidden"><div className="font-bold text-xs uppercase truncate">{rec.fileName}</div><div className="text-[9px] font-bold text-slate-500 uppercase">Secure View</div></div>
            </div>
          ))}
          {records.length === 0 && <div className="col-span-full py-10 text-center text-slate-700 font-bold text-[10px] uppercase tracking-widest border-2 border-dashed border-slate-800 rounded-3xl">Vault empty</div>}
        </div>
      </div>
    </div>
  );
